...
othmed = read_csv("E:/CU/RA/data/ESETT/Form 501c.csv")%>%
  ...
  mutate(omedt60 = omedt <= 60) %>% 
  left_join(form501) 
othmed_ = left_join(form501, othmed) %>% 
  mutate(omed = as.numeric(!is.na(omed_type))) %>% select(-ID, -omed_type) %>% 
...
intub1 = read_sas("E:/CU/RA/data/ESETT/form104.sas7bdat") %>% select(zSubjectID, F104Q34, F104AEName) %>% rename(id = zSubjectID, intubo60 = F104Q34, cause = F104AEName)
...
intub = left_join(intub2, intub1) 


intub = intub %>%
  mutate(add = sum + intubwi60) %>% 
  mutate(itb = add > 0) %>% select(id, itb)
...
data = data %>% 
  full_join(Age) %>% full_join(Gender) %>% full_join(Pre) %>% full_join(homemed_num) %>% full_join(homemed_match) %>% full_join(home_seizure_med) %>% full_join(benzo_dose) %>% full_join(othmed_) %>% full_join(intub)  %>% full_join(intcare)%>% full_join(success) %>% 
...
```{r}
data %>% group_by(age_group) %>% summarise(n = n())
```
